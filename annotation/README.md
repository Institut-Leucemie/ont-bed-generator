...
3. **PAR patch — one added line, fully sourced.** The NCBI `genomic.gff` build
   ...
   *is* annotated on both by NCBI). We add the missing chrX copy:
   ...
   - **Coordinates are not invented.** They are the alignment of NCBI's curated
     ...
     (`chrX:1304499-1338509`). Confirmed against HGNC (symbol `P2RY8` ↔
     `ENSG…` ↔ Entrez `286530`) and against the PAR1 gene lists.
...
- Tied to NCBI release `RS_2025_08`. Regenerate (and bump the asset version) when
  NCBI publishes a new annotation release — ideally after the upstream *P2RY8*
  chrX omission is fixed, at which point step 3 becomes unnecessary.
